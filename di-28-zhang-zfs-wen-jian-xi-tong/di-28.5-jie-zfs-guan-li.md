...
The FreeBSD Chinese Community (CFC) provides ZFS scripts that can be used to view, create, delete, and restore ZFS snapshots. [ZFS Script Project Repository](https://github.com/FreeBSD-Ask/zfs-snap). The script has been deployed at [https://docs.bsdcn.org/zfs.sh](https://docs.bsdcn.org/zfs.sh) and can be downloaded directly on a FreeBSD system using the `fetch` command.
