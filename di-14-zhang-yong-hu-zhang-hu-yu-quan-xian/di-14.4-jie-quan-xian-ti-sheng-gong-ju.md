...
## This file must be edited using the visudo command as root.

## Failure to use 'visudo' may result in syntax or file permission errors
## that prevent sudo from running.
## Not using visudo may result in syntax or permission errors, preventing sudo from running.

##
## See the sudoers man page for the details on how to write a sudoers file.
## For detailed instructions on how to write a sudoers file, see the sudoers manual page (man sudoers).
...
## Preserve editor-related environment variables for visudo.

## To preserve these for all commands, remove the "!visudo" qualifier.
## To preserve these variables for all commands, remove the "!visudo" qualifier.

Defaults!/usr/local/sbin/visudo env_keep += "SUDO_EDITOR EDITOR VISUAL"
## Only preserve environment variables SUDO_EDITOR, EDITOR, VISUAL for the /usr/local/sbin/visudo command.

##
## Use a hard-coded PATH instead of the user's to find commands.
## Use a hard-coded PATH to find commands instead of the user's PATH.

## This also helps prevent poorly written scripts from running
## arbitrary commands under sudo.
## This also helps prevent poorly written scripts from executing arbitrary commands under sudo.

Defaults secure_path="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
## Specify a fixed secure PATH for sudo.

##
## Uncomment if needed to preserve environmental variables related to the
## FreeBSD pkg utility and fetch.
## Uncomment if you need to preserve environment variables related to the FreeBSD pkg utility and fetch.

# Defaults     env_keep += "PKG_CACHEDIR PKG_DBDIR FTP_PASSIVE_MODE"
## Example: Preserve pkg cache/database paths and FTP passive mode variables.

##
## Additionally uncomment if needed to preserve environmental variables
## related to portupgrade
## Additionally, uncomment if you need to preserve environment variables related to portupgrade.

# Defaults     env_keep += "PORTSDIR PORTS_INDEX PORTS_DBDIR PACKAGES PKGTOOLS_CONF"
## Example: Preserve ports tree, index, database, binary packages, and tool configuration variables.

##
## You may wish to keep some of the following environment variables
## when running commands via sudo.
## When running commands via sudo, you may want to preserve some of the following environment variables.
...
## Example: Preserve language and LC_* locale-related variables.
...
## Example: Preserve X application resource search path-related variables.

##
## Desktop path settings
## Desktop-related path settings

# Defaults env_keep += "QTDIR KDEDIR"
## Example: Preserve Qt/KDE installation path variables (legacy Qt3/KDE3 era variables, no longer used in modern systems).

##
## Allow sudo-run commands to inherit the callers' ConsoleKit session
## Allow sudo-run commands to inherit the caller's ConsoleKit session.

# Defaults env_keep += "XDG_SESSION_COOKIE"
## Example: Preserve XDG session cookie (ConsoleKit era variable).

##
## Uncomment to enable special input methods.  Care should be taken as
## this may allow users to subvert the command being run via sudo.
## Uncomment to enable special input methods. Care should be taken as this may allow users to subvert the command being run via sudo.

# Defaults env_keep += "XMODIFIERS GTK_IM_MODULE QT_IM_MODULE QT_IM_SWITCHER"
## Example: Preserve input method-related environment variables.
...
## Commands run as non-root users will use a pseudo-terminal instead of the user's original terminal to prevent command injection.

# Defaults>root !use_pty
## Disable use_pty for root target user (example, commented by default).
...
## root can run any command on all hosts as any (user:group) identity (authentication required).

## Uncomment to allow members of group wheel to execute any command
## Uncomment to allow wheel group members to execute any command.

# %wheel ALL=(ALL:ALL) ALL
## Example: Grant wheel group sudo privileges similar to root (password required).

## Same thing without a password
## Same as above, but without entering a password.

# %wheel ALL=(ALL:ALL) NOPASSWD: ALL
## Example: Wheel group members can execute any command without a password.

## Uncomment to allow members of group sudo to execute any command
## Uncomment to allow sudo group members to execute any command.

# %sudo ALL=(ALL:ALL) ALL
## Example: Grant sudo group permission to execute any command as any (user:group) identity (password required).

## Uncomment to allow any user to run sudo if they know the password
## of the user they are running the command as (root by default).
## Uncomment to allow any user to run sudo if they know the target user's (default root) password.

# Defaults targetpw  # Ask for the password of the target user
## Use the target user's password for authentication (instead of the caller's password).

# ALL ALL=(ALL:ALL) ALL  # WARNING: only use this together with 'Defaults targetpw'
## All users can run any command as any identity (dangerous! Only use with Defaults targetpw).

## Read drop-in files from /usr/local/etc/sudoers.d
## Read additional configuration snippets (drop-in) from /usr/local/etc/sudoers.d.

@includedir sudoers.d
## Include the relative directory sudoers.d (relative to /usr/local/etc), reading configuration files from it.
```
