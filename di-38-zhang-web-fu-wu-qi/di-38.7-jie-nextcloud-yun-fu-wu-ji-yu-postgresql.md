...
CREATE ROLE
postgres=# \password nextcloud --Set or modify the password for user nextcloud; note that the backslash \ must be entered
...
CREATE DATABASE
postgres-# \q --Exit the PostgreSQL command line; note that the backslash must be entered: \
```
