# 35.1 Introduction to Databases
...
| `float` | 4B |
| `double` | 8B |
| `decimal(M, D)` | M is the total number of digits, D is the number of digits after the decimal point |
...
```sql
ALTER TABLE 表名
MODIFY COLUMN 列名 新数据类型; -- MySQL-specific syntax
...
update book_table
set Price = Price * 1.2
where Publisher = "人民邮电出版社"; -- MySQL allows double quotes for string values by default; in standard SQL and PostgreSQL, double quotes are for identifier quoting, and string values must use single quotes
...
This example multiplies the `Price` value of all records in the `book_table` where the `Publisher` value is "人民邮电出版社" by `1.2`, effectively increasing the price by 20%.
...
2. If the `author` value is 王阳 or 刘天洋, the corresponding operation is performed.
3. If the `name` value is 张三 or 李四, the corresponding operation is performed.
...
Example: Check whether a book title entry matches the pattern `《xxx 设计 yyy》`.
...
In a book management database, using LIKE '%设计%' would return book titles such as 《MySQL 数据库设计》.
...
```sql
SELECT * from `学生表` -- Backticks are MySQL-specific identifier quoting; standard SQL uses double quotes
ORDER BY `学号` desc
LIMIT 0,3;
```
